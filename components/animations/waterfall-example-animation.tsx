  ...
  return (
    ...
    >
      <path
        ...
        fill="currentColor"
        className="text-neutral-700 dark:text-neutral-300 group-hover:text-sky-700 dark:group-hover:text-sky-500 transition-all"
      />
    </svg>
  );
}

export default function WaterfallExampleAnimation({
  caption,
}: {
  caption: string;
}) {
  ...
  async function reqAnimation() {
    await reqAnimate(reqScope.current, { opacity: 1, translateY: "50%" });
    await reqAnimate(
      reqScope.current,
      { translateX: "-110%" },
      { duration: 3 },
  ...
  async function query1Animation() {
    await query1Animate(query1Scope.current, { opacity: 1 }, { duration: 0.3 });
  }

  async function query2Animation() {
    await query2Animate(query2Scope.current, { opacity: 1 }, { duration: 0.3 });
  }

  async function query3Animation() {
    await query3Animate(query3Scope.current, { opacity: 1 }, { duration: 0.3 });
  }

  async function queryResetAnimation() {
    await query1Animate(query1Scope.current, { opacity: 0 }, { duration: 0 });
    await query2Animate(query2Scope.current, { opacity: 0 }, { duration: 0 });
    await query3Animate(query3Scope.current, { opacity: 0 }, { duration: 0 });
  }

  async function resAnimation() {
    await resAnimate(resScope.current, { translateY: "-50%" });
    await resAnimate(
      resScope.current,
      { opacity: 1 },
      { duration: 0.5, delay: 1 },
    );
    await resAnimate(resScope.current, { translateX: "110%" }, { duration: 3 });
  }
    ...
    <div className="my-4" aria-hidden={"true"}>
      <div className="grid grid-cols-8 md:grid-cols-11 gap-x-2 text-center mb-4">
        ...
        <div className="col-span-2 md:col-span-3">
          ...
          <div className="hidden md:flex md:justify-center">
            (longest point of latency)
          </div>
        </div>
        <div className="col-span-3 md:col-span-4">
          <div>Client</div>
          <div className="hidden md:flex md:justify-center">
            (waiting on this entire <br />
            process before loading UI)
          </div>
        </div>
      </div>
      <div className="grid grid-cols-8 md:grid-cols-11 gap-x-2">
        <div className="flex flex-col justify-center col-span-3 md:col-span-4">
          <div className="dark:bg-neutral-950 bg-neutral-50 border-2 border-yellow-400 dark:border-yellow-600 h-full w-full rounded-xl grid place-items-center relative z-40">
            <svg
              width="90%"
              height="80%"
              className="text-neutral-500 dark:text-neutral-300 relative"
              viewBox="0 0 28 28"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              ...
              />
              <path
                d="M0 1.03226H0.428571V6.70968H0V1.03226Z"
                fill="currentColor"
              />
              <path
                d="M5.57143 1.03226H6V6.70968H5.57143V1.03226Z"
                fill="currentColor"
              />
              <path
                fillRule="evenodd"
              ...
              />
              <path
                d="M0 11.0323H0.428571V16.7097H0V11.0323Z"
                fill="currentColor"
              />
              <path
                d="M5.57143 11.0323H6V16.7097H5.57143V11.0323Z"
                fill="currentColor"
              />
              <path
                fillRule="evenodd"
              ...
              />
              <path
                d="M0 21.0323H0.428571V26.7097H0V21.0323Z"
                fill="currentColor"
              />
              <path
                d="M5.57143 21.0323H6V26.7097H5.57143V21.0323Z"
                fill="currentColor"
              />
              <path
                fillRule="evenodd"
              ...
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M14 12.5H28V16.5H14V12.5ZM14.5 13V16H27.5V13H14.5Z"
                fill="currentColor"
              />
              <path
                fillRule="evenodd"
              ...
              />
              <path
                d="M25.5 11C25.5 11.2761 25.2761 11.5 25 11.5C24.7239 11.5 24.5 11.2761 24.5 11C24.5 10.7239 24.7239 10.5 25 10.5C25.2761 10.5 25.5 10.7239 25.5 11Z"
                fill="currentColor"
              />
              <path
                d="M25.5 14.5C25.5 14.7761 25.2761 15 25 15C24.7239 15 24.5 14.7761 24.5 14.5C24.5 14.2239 24.7239 14 25 14C25.2761 14 25.5 14.2239 25.5 14.5Z"
                fill="currentColor"
              />
              <path
                d="M25.5 18C25.5 18.2761 25.2761 18.5 25 18.5C24.7239 18.5 24.5 18.2761 24.5 18C24.5 17.7239 24.7239 17.5 25 17.5C25.2761 17.5 25.5 17.7239 25.5 18Z"
                fill="currentColor"
              />
            </svg>
          </div>
        </div>
        <div className="flex flex-col justify-center col-span-2 md:col-span-3 text-neutral-500 dark:text-neutral-300 relative z-0">
          <svg
            className="relative"
            width="100%"
            height="100%"
            ...
            <div ref={reqScope} className="opacity-0 translate-y-[50%]">
              ...
              >
                <circle
                  ...
                  r="2"
                  fill="currentColor"
                  className="text-green-500 stroke-[0.3] stroke-green-700"
                />
              </svg>
            </div>
            <div
              ref={resScope}
              className="opacity-0 -translate-y-[50%] -translate-x-[10%]"
            >
              <svg
                width="100%"
                height="100%"
        ...
        <div className="flex flex-col justify-center text-neutral-300 dark:text-neutral-600 col-span-3 md:col-span-4">
          <div className="dark:bg-neutral-950 bg-neutral-50 border-2 border-neutral-300 dark:border-neutral-600 h-full w-full rounded-xl p-3 px-0 relative">
            <div className="flex w-full rounded-t-xl justify-between border-b-2 border-neutral-300 dark:border-neutral-600 relative">
      ...
      <div className="flex items-center gap-x-6 mt-4">
        <div>
          <div className="w-12 h-[56px] bg-neutral-300 border-neutral-300 dark:bg-neutral-600 dark:border-neutral-600 rounded-xl md:rounded-2xl border-2 absolute" />
          <button
            onClick={handleClick}
            disabled={isPlaying}
            aria-label="play animation"
            className="group w-12 h-12 bg-neutral-100 border-neutral-300 dark:bg-neutral-800 dark:border-neutral-600 rounded-xl md:rounded-2xl border-2 grid place-items-center hover:bg-neutral-200 dark:hover:bg-neutral-700 disabled:bg-neutral-100 dark:disabled:bg-neutral-800 active:scale-90 transition-all duration-150 relative text-neutral-700 dark:text-neutral-300"
          >
            <>{isPlaying ? <Spinner /> : <PlayButton />}</>
          </button>
        </div>
        <div className="flex items-center text-sm text-center mt-4">
          {caption}
        </div>
      </div>
    </div>
