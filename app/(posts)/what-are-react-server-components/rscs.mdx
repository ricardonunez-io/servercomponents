...
The difference, then, between React Server Components is that the process SSR takes to render your React app is:
...
That is to say the browser is getting the serialized output of your JSX directly from the server, instead of having to transpile the JSX itself on the client.

That's certainly a lot to keep track of, but Ryan Florence, one of the creators of [<ReactRouterLogo width={30}/> React Router](https://reactrouter.com) and [<RemixLogo width={20} /> Remix](https://remix.run), had a great Tweet to explain this mental model:
