...
What pre-rendering means is that we're essentially rendering your React component tree as HTML and sending it to the browser, using React's `renderToString(){:js}` method. That string of HTML which looks like this:
...
This string of HTML is then sent to the browser to render, and it's then hydrated by React once the browser loads React and all the necessary Javascript.
