...
import RailsIcon from "@/components/rails-icon";
import PHPLogo from "@/components/php-logo"
import DiffExample from "@/components/diff-example";
...
However, the key difference here is that data changes don't trigger a reload of the entire page, and this is one of they key improvements over `getServerSideProps` and Remix `loaders`.
...
A. Won't have to reload to get those new comments.

B. Won't lose their search input if they navigate to a new blog post (provided the search bar is part of a layout that wraps the comments and blog post)
