...
Static in this context means "not interactive", not that they're not dynamic. You can still pass props to these components, you can still render dynamic data, but you can't use hooks or other React features **that require a browser**.
...
Well, it's still there, but now, "regular" React components (i.e. React pre-Server Components) are called "client components". Only the name changed, though, but they're exactly the same and nothing at all has changed about them in terms of functionality.
